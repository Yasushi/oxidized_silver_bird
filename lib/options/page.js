...
$(function() {
  ...
  var onShortenerAcctClick = function() {
    if($("input[name='shortener_acct']").is(':checked')) {
      $("input[name='shortener_login']").prop('disabled', false);
      $("input[name='shortener_key']").prop('disabled', false);
    } else {
      $("input[name='shortener_login']").val('').prop('disabled', true);
      $("input[name='shortener_key']").val('').prop('disabled', true);
    }
  };
  ...
  var onSigningUrlCheck = function() {
    ...
    if($check.is(':checked')) {
      $("input[name='base_signing_url'], input[name='base_oauth_signing_url']").prop('disabled', true);
      ...
      $("input[name='base_signing_url'], input[name='base_oauth_signing_url']").prop('disabled', false);
  ...
  $("input[name='microblogging_service'],label[for='service_twitter']").mousedown(function(e) {
    var el = $(e.target);
    if(el.is('label')) {
      el = $('#' + el.attr('for'));
    }
  ...
  $("#nerds_link").click(function() {
    var table = $("#rate_limit_status");
    table.toggle();
    if (!table.is(':visible')) {
      return;
    }
    var cb = function(success, data, status, context, request) {
      if (!success) {
        return;
      }
      ...
      for (var resource in data['resources']) {
        var rs = data['resources'][resource];
        $(`<tr style="background-color: #ddd"><td class="label" colspan="3">${resource}</td></tr>`).appendTo(tbody);
        for (var endpoint in rs) {
          var r = rs[endpoint];
          var limit = r['limit'];
          var remaining = r['remaining'];
          var tr = $("<tr>");
          tr.append($("<td class='label'>").text(endpoint));
          var rl = $("<td>").text(`${remaining}/${limit}`);
          if (remaining !== limit) {
            rl.css('background-color', '#ff0');
          }
          tr.append(rl);
          tr.append($("<td>").text(new Date(r['reset'] * 1000).toLocaleString()));
          tr.appendTo(tbody);
        }
      }
    };
    chrome.extension.getBackgroundPage().TweetManager.instance.twitterBackend.rateLimitStatus(cb);
  });
});
...
function updatePredictedHitsCount() {
  ...
  TimelineTemplate.eachTimelineTemplate(function(template) {
    ...
    inputUnifiedEl.prop('disabled', !unifiedVisible);
