...
var Renderer = {
  ...
  entitiesFuncs: {
    ...
    handleLink: function(link, entity) {
      ...
      if (entity.sizes) {
        $(link).tooltip({
            ...
            content: function() {
              return $("<img>").attr('src',
                (entity.media_url_https||entity.media_url) +":thumb");
            }
        });
      } else {
        // TODO expand specific url
        $(link).tooltip();
      }
    }
  },
  ...
  parseEntities: function(text, entities) {
    ...
    for (i = 0, len = orderedEntities.length; i < len; ++i) {
      ...
      if (entity.type === 'mention') {
        ...
        this.entitiesFuncs.handleLink(link, entity);
  ...
  makeSpan: function (content) {
    return $("<span>").text(content)[0];
  },

  makeElem: function(elem, attrs, content) {
    return $("<"+elem+">", attrs).text(content)[0];
  },

  makeDiv: function(attrs, content) {
    return $("<div>", attrs).text(content)[0];
  },

  makeImage: function(src, options) {
    return $("<img>", options).attr('src',src)[0];
  },
  ...
  renderTweet: function (tweet, useColors, nameAttribute) {
    ...
    if(tweet.source) {
      ...
      var htmlNode = $("<span>").html(tweetSource);
      if(tweetSource.indexOf('&lt;') === 0) {
        ...
        console.warn("tweet.source invalid escaped. (" + tweetSource + ")");
      }
      // $("a", htmlNode).filter("[href^='h']").map(function(i,e){return jQuery(e).attr('href')})
      var sourceA = $("a", htmlNode);
      var sourceHref = sourceA.attr('href');
      var sourceText = sourceA.text();
      if(sourceHref && sourceHref.length > 0) {
        from = $("<a>", {href: 'javacript:'}).text(sourceText)[0];
        AnyClick.anyClick(from, function() { openTab(sourceHref); });
      } else {
        from = Renderer.makeText(sourceText);
      }
    }
    if(tweet.place || tweet.geo || tweet.coordinates) {
      var place_name = (tweet.place||{}).full_name;
      geo = Renderer.makeSpan();
      ...
      geo.appendChild(Renderer.makeText(place_name));
    }
