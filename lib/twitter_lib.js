...
function TwitterLib(onAuthenticated, microbloggingService, baseUrl, baseOauthUrl, baseSigningUrl, baseOauthSigningUrl, baseSearchUrl, oauthTokenData) {
...
TwitterLib.prototype = {
    ...
    var errorCallback = function (request, status, error) {
      ...
      if(!fmtError) {
        try {
          ...
          if(_this.microbloggingService == 'twitter') {
  ...
  // alias id_str to id
  aliasStringNumProps: function(ti) {
    if (!ti) {
      return;
    }
    ...
    for (var prop in ti) {
      ...
      if (typeof ti[prop] === 'object') {
        this.aliasStringNumProps(ti[prop]);
        continue;
      }
  ...
  rateLimitStatus: function(callback) {
    this.ajaxRequest('application/rate_limit_status', callback, null, {}, "GET");
  },
  ...
  trendingTopics: function(callback, param) {
    ...
    this.ajaxRequest('trends/place', callback, null, {id: param}, "GET");
  },

  availableTrends: function(callback) {
    this.ajaxRequest('trends/available', callback, null, {}, "GET");
  },
...
TwitterOAuth.prototype = {
  ...
  requestTokenCallback: function(data, status, tryAgain) {
    ...
    var alertRequestError = function(errorMsg) {
      ...
      alert("Ouch... Something bad happened to Oxidized Silver Bird while calling Twitter's API. Request token response: " + errorMsg +
  ...
  getRequestToken: function() {
    ...
    this.makeRequest('request_token', {"oauth_callback": "oob"}, this.requestTokenCallback);
  }
};
